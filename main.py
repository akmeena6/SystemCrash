...
from stuff import handle, model
...
async def register(data: model.User_For_Registration):
    return await handle.handle_register(data)
...
@app.post("/login")
async def login(data: model.User_Login):
    return await handle.login(data.user_id, data.password)
